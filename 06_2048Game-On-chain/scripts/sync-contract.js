...
const abiSource = path.join(
  outDir,
  "OnChain2048Scores.sol",
  "OnChain2048Scores.json"
);
const abiTarget = path.join(
  frontendDir,
  "lib",
  "generated",
  "onchain2048scores-abi.json"
);
...
const parseArgs = () => {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--score-address") {
      parsed.scoreAddress = next;
      i += 1;
    } else if (arg === "--rpc-url") {
      parsed.rpcUrl = next;
      i += 1;
  ...
  for (const filePath of files) {
    try {
      ...
      for (const tx of parsed.transactions ?? []) {
        ...
        if (contractName === "OnChain2048Scores" && isAddress(contractAddress)) {
          console.log(
            `Score contract inferred from ${path.relative(rootDir, filePath)}`
          );
...
const readAbi = () => {
  ...
  if (!Array.isArray(artifact.abi)) {
    throw new Error("OnChain2048Scores artifact does not contain a valid ABI.");
  }
  return artifact.abi;
};
...
const main = () => {
  ...
  const scoreAddress =
    cliArgs.scoreAddress ||
    readAddressFromBroadcast() ||
    runtimeConfig.scoreContractAddress ||
    envConfig.NEXT_PUBLIC_SCORE_CONTRACT_ADDRESS ||
    ZERO_ADDRESS;
  ...
  const resolvedScoreAddress = isAddress(scoreAddress) ? scoreAddress : ZERO_ADDRESS;
  ensureDir(runtimeConfigFile);
  fs.writeFileSync(
    runtimeConfigFile,
    `${JSON.stringify(
      {
        scoreContractAddress: resolvedScoreAddress,
        rpcUrl,
        chainId,
      },
      null,
  ...
  mergeEnvFile({
    NEXT_PUBLIC_SCORE_CONTRACT_ADDRESS: resolvedScoreAddress,
