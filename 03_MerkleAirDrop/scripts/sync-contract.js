...
const abiSources = [
  ...
  {
    contractName: "LLCAirDrop",
    source: path.join(contractsDir, "out", "LLCAirDrop.sol", "LLCAirDrop.json"),
    target: path.join(frontendDir, "LLCAirDrop.json"),
  },
];
...
const parseArgs = () => {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--airdrop-address") {
      parsed.airdropAddress = next;
      index += 1;
      continue;
    }
  ...
  for (const file of files) {
    try {
      ...
      for (const transaction of parsed.transactions || []) {
        ...
        if (contractName === "LLCAirDrop" && !output.airdropAddress) {
          output.airdropAddress = contractAddress;
        }
      }

      if (output.luluCoinAddress || output.airdropAddress) {
...
const main = () => {
  ...
  const airdropAddress = normalizeAddress(
    args.airdropAddress ||
      broadcast.airdropAddress ||
      runtimeConfig.airdropAddress ||
      frontendEnv.NEXT_PUBLIC_AIRDROP_ADDRESS ||
      contractsEnv.AIRDROP_CONTRACT,
    ZERO_ADDRESS
  );
  ...
  fs.writeFileSync(
    runtimeConfigTarget,
    `${JSON.stringify({ luluCoinAddress, airdropAddress, rpcUrl, chainId }, null, 2)}\n`
  );
  ...
  mergeEnvWithKnownKeys(contractsEnvTarget, {
    ...
    AIRDROP_CONTRACT: airdropAddress,
  });
