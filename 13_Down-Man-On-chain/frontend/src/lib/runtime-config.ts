...
type RuntimeConfigSource = {
  downManScoreboardAddress?: unknown
  address?: unknown
  rpcUrl?: unknown
  chainId?: unknown
}

export type DownManRuntimeConfig = {
  downManScoreboardAddress?: `0x${string}`
  rpcUrl: string
  chainId: number
}

let runtimeConfigCache: DownManRuntimeConfig | null = null
let runtimeConfigLoadPromise: Promise<DownManRuntimeConfig> | null = null
...
): DownManRuntimeConfig => ({
  downManScoreboardAddress: asAddress(
    runtimeConfig?.downManScoreboardAddress ?? runtimeConfig?.address,
  ),
  rpcUrl: asRpcUrl(runtimeConfig?.rpcUrl),
  chainId: asChainId(runtimeConfig?.chainId),
})

const cacheRuntimeConfig = (runtimeConfig: DownManRuntimeConfig): DownManRuntimeConfig => {
  runtimeConfigCache = normalizeRuntimeConfig(runtimeConfig)
  return runtimeConfigCache
}

const getEnvRuntimeConfig = (): DownManRuntimeConfig =>
  normalizeRuntimeConfig({
    downManScoreboardAddress: import.meta.env.VITE_DOWNMAN_ADDRESS,
    rpcUrl: import.meta.env.VITE_RPC_URL,
    chainId: import.meta.env.VITE_CHAIN_ID,
  })

export const getResolvedRuntimeConfig = (): DownManRuntimeConfig =>
  runtimeConfigCache ?? getEnvRuntimeConfig()

export const loadRuntimeConfig = async (): Promise<DownManRuntimeConfig> => {
