...
const abiSource = path.join(
  outDir,
  "LightsOutResults.sol",
  "LightsOutResults.json"
);
...
const parseArgs = () => {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--lights-out-address") {
      parsed.lightsOutAddress = next;
      i += 1;
    } else if (arg === "--rpc-url") {
      parsed.rpcUrl = next;
      i += 1;
  ...
  for (const filePath of files) {
    try {
      ...
      for (const tx of parsed.transactions ?? []) {
        ...
        if (contractName === "LightsOutResults" && isAddress(contractAddress)) {
          console.log(
            `LightsOut address inferred from ${path.relative(rootDir, filePath)}`
          );
...
const readAbi = () => {
  ...
  if (!Array.isArray(artifact.abi)) {
    throw new Error("LightsOutResults artifact does not contain a valid ABI.");
  }
  return artifact.abi;
};
...
const main = () => {
  ...
  const lightsOutAddress =
    cliArgs.lightsOutAddress ||
    readAddressFromBroadcast() ||
    runtimeConfig.lightsOutAddress ||
    envConfig.NEXT_PUBLIC_LIGHTS_OUT_ADDRESS ||
    ZERO_ADDRESS;
  ...
  const resolvedAddress = isAddress(lightsOutAddress)
    ? lightsOutAddress
    : ZERO_ADDRESS;
  ...
  fs.writeFileSync(
    runtimeConfigFile,
    `${JSON.stringify(
      { lightsOutAddress: resolvedAddress, rpcUrl, chainId },
      null,
      2
    )}\n`
  );
  ...
  mergeEnvFile({
    NEXT_PUBLIC_LIGHTS_OUT_ADDRESS: resolvedAddress,
