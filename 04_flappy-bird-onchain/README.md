# Flappy Bird On-chain（React + Phaser + Foundry）

一个以 React 作为宿主、Phaser 作为游戏引擎的 Flappy Bird 变体，并集成了 wagmi 钱包连接与链上排行榜。前端位于 `game-frontend/`，合约工程独立在 `contracts/`（Foundry）。
...
- `VITE_FLAPPY_SCORE_ADDRESS`：合约地址（`make deploy` 会写入 `game-frontend/.env.local`）
- `VITE_ANVIL_RPC_URL`（可选）：自定义 RPC URL（默认 `http://127.0.0.1:8545`）

地址读取优先级：`VITE_FLAPPY_SCORE_ADDRESS` > `game-frontend/components/Web3/flappyScore.address.json`。
...
### 启动与渲染
1. `game-frontend/index.html` 提供 `#root`
