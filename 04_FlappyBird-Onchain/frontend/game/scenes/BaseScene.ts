...
import { ensureAudioAssets, syncBgm } from "../audio/audioManager";
...
class BaseScene extends Phaser.Scene {
  ...
  ensureAudioUnlocked() {
    ...
    if (this.game.registry.get("audioUnlocked")) {
      void ensureAudioAssets(this).then(() => {
    ...
    const unlock = () => {
      ...
      // 解锁后再按需加载音频，避免首屏 Preload 阶段抢占资源。
      void ensureAudioAssets(this).then(() => {
        syncBgm(this, loadSettings());
      });
    };
