...
export type RuntimeConfig = {
  bookManagementAddress: `0x${string}`;
  rpcUrl: string;
  chainId: number;
};
...
const envConfig: RuntimeConfig = {
  bookManagementAddress: isAddress(process.env.NEXT_PUBLIC_BOOK_MANAGEMENT_ADDRESS)
    ? process.env.NEXT_PUBLIC_BOOK_MANAGEMENT_ADDRESS
    : isAddress(process.env.NEXT_PUBLIC_CONTRACT_ADDRESS)
...
export function getRuntimeConfig(): RuntimeConfig {
  ...
  return {
    bookManagementAddress: isAddress(runtime.bookManagementAddress)
      ? runtime.bookManagementAddress
      : envConfig.bookManagementAddress,
