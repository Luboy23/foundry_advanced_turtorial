...
import { readSellerOrdersSnapshot } from "@/lib/server/order-snapshots";
...
export async function GET(request: Request) {
  try {
    ...
    const orders = await readSellerOrdersSnapshot(publicClient, config, address as Address);
    return NextResponse.json({ orders: serializeOrders(orders) });
  } catch (error) {
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "当前未能读取卖家订单，请稍后重试。"
      },
      { status: 500 }
    );
  }
